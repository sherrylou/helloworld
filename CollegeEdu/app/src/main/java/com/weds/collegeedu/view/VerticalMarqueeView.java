package com.weds.collegeedu.view;
...
public class VerticalMarqueeView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private VerticalMarqueeView.MarqueeViewThread mThread;
    ...
    private StaticLayout staticLayout;//绘制多行文本
...
//        int height = wm.getDefaultDisplay().getHeight();
        ...
        if (mStartPoint == 0)
            ...
            currentY = height;
//        LogUtils.i("竖直滚动通知", textWidth + "---" + textHeight + "---" + height + "---" + currentY + "---" + margueeString);
    }
     ...
     * @param isStop
     */
    public void startScroll(boolean isStop) {
        this.isStop = isStop;
//        if (mThread != null && mThread.isRun && !mThread.isInterrupted())
//            return;
        ...
        isFirstDraw = true;
//        isFirstMeasureHeight = false;
        mThread = new VerticalMarqueeView.MarqueeViewThread(holder);//创建一个绘图线程
        mThread.isRun = true;
        mThread.start();
    }
...
//            mThread.interrupt();
    ...
    public void clearText() {
        if (mThread != null && mThread.isRun) {
            LogUtils.i("noticelxy", "-1--:-------------");
            margueeString = "       1                 ";
//            mHandler.sendEmptyMessageDelayed(STOP_THREAT, 1000);
        }
    }
            ...
            try {
                synchronized (holder) {
                    ...
                    if (isGo) {
                        ...
                        LogUtils.i("竖直滚动通知", "---宽度" + getWidth() + "---");
                        if (canvas != null) {
                            canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);//绘制透明色
                            textCenter(margueeString, mTextPaint, canvas, point, getWidth(), Layout.Alignment.ALIGN_NORMAL, 1.5f, 0, false, currentY);
                            holder.unlockCanvasAndPost(canvas);//结束锁定画图，并提交改变。
                            LogUtils.i("竖直滚动通知", "---" + isFirstDraw + "---" + margueeString);
                            if (isFirstDraw) {
                                mHandler.sendEmptyMessageDelayed(STOP_ROLL, 50);
                                isFirstDraw = false;
                            }
                            LogUtils.i("竖直滚动通知", textWidth + "---" + textHeight + "---" + currentY);
                        }
                        Thread.sleep(80);//睡眠时间为移动的频率
                    }
                }
            } catch (Exception e) {
                LogUtils.e("竖直滚动通知", e.toString());
                e.printStackTrace();
            }
        }
...
//     * 第一次测量
//     */
//    private boolean isFirstMeasureHeight = true;
    ...
    private void textCenter(String string, TextPaint textPaint, Canvas canvas, Point point, int width,
                            Layout.Alignment align, float spacingmult, float spacingadd, boolean includepad, int height) {
        canvas.save();
    ...
    public static final int STOP_ROLL = 200;
    public static final int START_ROLL = 300;
    public static final int STOP_THREAT = 400;
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case STOP_ROLL:
                    isGo = false;
                    mHandler.sendEmptyMessageDelayed(START_ROLL, 5000);
                    break;
                case START_ROLL:
                    isGo = true;
                    break;
                case STOP_THREAT:
                    if (margueeString.equals("                        ")) {
                        stopScroll();
                    }
                    break;
            }
    ...
    VerticalMarqueeView.OnMargueeListener mOnMargueeListener;

    public void setOnMargueeListener(VerticalMarqueeView.OnMargueeListener mOnMargueeListener) {
